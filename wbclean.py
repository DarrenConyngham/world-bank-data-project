...
def flourish(indicator, start_yr, end_yr, country='all', save_csv=True):
    """
...
def plot_line_chart(df, title='Graph', figure_size=(18,10), ylim=None, save=False):
    """
    ...
    Parameters:
    * df: The dataframe in the correct format (i.e. you have run wbclean.get_wb_data() and assigned it).
    ...
    * save: Saves the graph to your current working directory.
    ...
    sns.lineplot(x=df.index, y=indicator_name, data=df, hue='Region', alpha=0.7)
...
def plot_bar_chart(df, title='Graph', year=None, margin=1.1, figure_size=(18,10), barcolour='darkslategray',
    ...
    * save: Saves the graph to your current working directory.
    ...
    if year == None:
        year = df.index.max()
    df = df[df.index == year].sort_values(by='Region')
    ...
    sns.barplot(x=indicator_name, y='Region', data=df.sort_values(by=indicator_name, ascending=False), color=barcolour)

    if decimals > 0:
        temp_df = df[indicator_name].sort_values(ascending=False).values.round(decimals).astype(float)
    else:
        temp_df = df[indicator_name].sort_values(ascending=False).values.round(decimals).astype(int)
    ...
    plt.xlabel(f'{title}')
    plt.ylabel('')
    plt.xlim(right = df[indicator_name].max() * margin)
